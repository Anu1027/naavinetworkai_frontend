...
function VaultPageContextProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (trackerType == "withdraw") {
      getTracketData(
        `https://comms.globalxchange.io/coin/vault/service/path/withdraw/txn/get?app_code=TaxChains&email=${email}`
      );
    } else if (assetDropdownSelected && statusDropdownSelected) {
      getTracketData(
        `https://comms.globalxchange.io/coin/vault/service/path/deposit/txn/get?email=${email}&status=${statusDropdownSelected}&app_code=TaxChains&coin=${assetDropdownSelected}`
      );
    } else if (assetDropdownSelected) {
      getTracketData(
        `https://comms.globalxchange.io/coin/vault/service/path/deposit/txn/get?email=${email}&app_code=TaxChains&coin=${assetDropdownSelected}`
      );
    } else if (statusDropdownSelected) {
      getTracketData(
        `https://comms.globalxchange.io/coin/vault/service/path/deposit/txn/get?email=${email}&status=${statusDropdownSelected}&app_code=TaxChains`
      );
    } else {
      getTracketData(
        `https://comms.globalxchange.io/coin/vault/service/path/deposit/txn/get?email=${email}&app_code=TaxChains`
      );
    }
