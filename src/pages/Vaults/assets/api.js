...
export function getMoneyMarketsVaults(
  ...
  axios
    .get(
      `https://comms.globalxchange.io/coin/vault/service/user/app/interest/balances/get?email=${email}&app_code=TaxChains&with_balances=${balanceToggle}&orderby_dsc=true`
    )
    .then((response) => {
      let res = response?.data?.result;
      if (res?.length > 0) {
